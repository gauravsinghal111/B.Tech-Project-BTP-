...
# 1. LOAD DATA
# ─────────────────────────────────────────────
train_raw = pd.read_csv("Training Set.csv")
test_raw  = pd.read_csv("Test Set.csv")

# Drop completely empty columns (test set has trailing blanks)
train_raw = train_raw.dropna(axis=1, how="all")
test_raw  = test_raw.dropna(axis=1, how="all")

# Drop rows with any NaN (only 1 in training)
...
plt.suptitle("Model Comparison on Test Set", fontsize=14, fontweight="bold", y=1.01)
plt.tight_layout()
plt.savefig("01_model_comparison.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("\nSaved: 01_model_comparison.pdf")
...
ax.set_title("5-Fold Cross-Validation R² (Training Set)", fontweight="bold")
ax.set_ylabel("CV R²")
ax.set_xticklabels(model_names, rotation=35, ha="right", fontsize=9)
plt.tight_layout()
plt.savefig("02_cv_scores.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 02_cv_scores.pdf")
...
plt.suptitle("Actual vs Predicted LD — All Models", fontsize=13, fontweight="bold", y=1.01)
plt.tight_layout()
plt.savefig("03_actual_vs_predicted_all.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 03_actual_vs_predicted_all.pdf")

# --- 7d. Best model – detailed actual vs predicted ---
...
plt.tight_layout()
plt.savefig("04_best_model_detail.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 04_best_model_detail.pdf")
...
for bar, val in zip(bars, sorted_vals):
    ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.002,
            f"{val:.3f}", ha="center", va="bottom", fontsize=8)
plt.tight_layout()
plt.savefig("05_feature_importance.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 05_feature_importance.pdf")

# --- 7f. LD Distribution ---
fig, axes = plt.subplots(1, 2, figsize=(12, 4))
axes[0].hist(y_train, bins=40, color="#3498db", edgecolor="white", alpha=0.85)
axes[0].set_title("LD Distribution — Training Set", fontweight="bold")
axes[0].set_xlabel("LD"); axes[0].set_ylabel("Count")

axes[1].hist(y_test, bins=25, color="#e67e22", edgecolor="white", alpha=0.85)
axes[1].set_title("LD Distribution — Test Set", fontweight="bold")
axes[1].set_xlabel("LD"); axes[1].set_ylabel("Count")

plt.tight_layout()
plt.savefig("06_ld_distribution.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 06_ld_distribution.pdf")

# --- 7g. Correlation heatmap (training set) ---
...
ax.set_title("Pearson Correlation Heatmap (Training Set)", fontweight="bold")
plt.tight_layout()
plt.savefig("07_correlation_heatmap.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 07_correlation_heatmap.pdf")

# --- 7h. Parity plot coloured by error magnitude ---
...
ax.legend(fontsize=9)
plt.tight_layout()
plt.savefig("08_parity_plot.pdf", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: 08_parity_plot.pdf")
